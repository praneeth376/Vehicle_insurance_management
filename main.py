...
#dsn_tns = cx_Oracle.makedsn('localhost', '1521', service_name='SYS$USERS')
...
class agent_insert:
    def __init__(self):
        ...
        top.mainloop()
    def insert(self):
        if self.nameins.get()=='' or self.addrins.get()=='' or self.phoneins.get()=='' or self.passwordins.get()=='':
...
class agent_login:
    def start(self,agent_key):
        ...
        ttk.Button(top,text="LOGOUT",command=self.logout).place(x=1150,y=40)
        ...
        self.treeview = ttk.Treeview(self.frame,height=7)
        ...
        Label(self.frame, text='Basic Insurance Policy', font=('arial 12'), fg='Black',bg="lightgreen").place(x=575, y=50)
...
class new_customer:
    ...
    def insert(self):
        if self.name.get()=='' or self.mobile.get()=='' or self.address.get()=='' or self.desc.get()=='' or self.number.get()=='' or self.type.get()=='':
            a='Please enter correct details...'
            messagebox.showinfo("success",a)
        else:
...
class edit_customer:
    def start(self,agent_key):
        ...
        ttk.Label(self.frame, text="EDIT CUSTOMER DETAILS", font=('arial 15'),style="BW.TLabel").place(x=75, y=50)
    ...
    def edit_mobile(self):
        a = 'Info edited successfully'
        cur = con.cursor()
        statement = "UPDATE customer SET MOBILE=:3  WHERE custid=:1"
    ...
    def edit_address(self):
        a = 'Info edited successfully'
        cur = con.cursor()
        statement = "UPDATE customer SET ADDRESS=:4  WHERE custid=:1"
    ...
    def edit_desc(self):
        a = 'Info edited successfully'
        cur = con.cursor()
        statement = "UPDATE vehicle SET VEH_DESC=:3  WHERE cust_id=:2"
    ...
    def edit_vehno(self):
        a = 'Info edited successfully'
        cur = con.cursor()
        statement = "UPDATE vehicle SET VEH_num=:4  WHERE cust_id=:2"
    ...
    def edit_type(self):
        a = 'Info edited successfully'
        cur = con.cursor()
        statement = "UPDATE vehicle SET VEH_TYPE=:5  WHERE cust_id=:2"
...
class Admin_Page:
    def __init__(self):
        ...
        self.right = Frame(top, width=480, height=720, bg="lightpink").pack(side=LEFT)
        ...
        Label(self.left,text="ADMINISTRATOR LOGIN",font=('arial 30 bold'),fg='Black',bg='#f55a33').place(x=0,y=0)
        ttk.Button(self.left,text="NEW AGENT",command=self.new_agent).place(x=80,y=160)
        ttk.Button(self.left,text="DELETE AGENT",command=self.delete).place(x=80,y=240)
        ttk.Entry(self.left,textvariable=self.agent_key).place(x=200,y=240)



        Label(self.right, text="EDIT AGENT DETAILS", font=('arial 15'), fg='Black', bg='#41acf4').place(x=870,y=50)
        ...
        ttk.Button(self.right, text="Update Address",command=self.edit_address).place(x=1000, y=223)
        #agent phone
        ttk.Entry(self.right, textvariable=self.phone).place(x=850, y=275)
        ttk.Button(self.right, text="Update Phone No.",command=self.edit_phone).place(x=1000, y=273)
        #agent password
        ttk.Entry(self.right, textvariable=self.password).place(x=850, y=325)
        ttk.Button(self.right, text="Update Password" ,command=self.edit_pwd).place(x=1000, y=323)

        ttk.Button(self.right, text="LOGOUT",command=self.logout).place(x=1150,y=24)
    ...
    def delete(self):
        a = 'Record deleted successfully with agent id =' + str(self.agent_key.get())+' Please Login again....!'
        cur = con.cursor()
    ...
    def stop(self):
        ...
        #edit features for agent
        #(agent_key,name,address,phone,pwd) values(:2,:3,:4,:5,:6)

    def edit_name(self):
        a = 'Success.... Please login again!'
        cur = con.cursor()
        statement = "UPDATE agent SET NAME=:2  WHERE AGENT_KEY=:1"
    ...
    def edit_phone(self):
        a = 'Success.... Please login again!'
        cur = con.cursor()
        statement = "UPDATE agent SET PHONE=:4  WHERE AGENT_KEY=:1"
        cur.execute(statement, (self.phone.get(),self.agent_key_edit.get()))
        messagebox.showinfo("Success...", a)
        cur.close()
        con.commit()
    ...
    def edit_address(self):
        a = 'Success.... Please login again!'
        cur = con.cursor()
        statement = "UPDATE agent SET ADDRESS=:3  WHERE AGENT_KEY=:1"
        cur.execute(statement, (self.address.get(),self.agent_key_edit.get()))
        messagebox.showinfo("Success", a)
        cur.close()
        con.commit()
    ...
    def edit_pwd(self):
        a = 'Success.... Please login again!'
        cur = con.cursor()
        statement = "UPDATE agent SET pwd=:5  WHERE AGENT_KEY=:1"
        cur.execute(statement, (self.password.get(),self.agent_key_edit.get()))
        messagebox.showinfo("Success", a)
        cur.close()
        con.commit()
...
class login:
    def __init__(self):
        ...
        ttk.Button(self.left,text="LOGIN",command=self.admin_login).place(x=88,y=180)
...
class start:
    def __init__(self):
        ...
        filename = PhotoImage(file=r"C:\Users\pc\Desktop\EDUCATION\DBMS\Vehicle-Insurance-Management-System-master\Vehicle-Insurance-Management-System\vehicle.png")
        background_label = Label(root, image=filename)
        background_label.place(x=0, y=0, relwidth=1, relheight=1)

        Button(root, text='click here to continue', background="#66c7df", font=('arial 14'),command=self.login).place(x=500, y=10)
        root.mainloop()
    def login(self):
        self.root.destroy()
        login()
